...
import { UnauthorizedChannelCreateDetector } from '../../src/core/runtime/discord/unauthorized-channel-create-detector';
...
function buildEvent(overrides: Partial<DiscordGatewayNormalizedEvent> = {}): DiscordGatewayNormalizedEvent {
  return {
    ...
    correlationId: 'corr-channel-create-1',
    payload: {
      guildId: 'guild-1',
      actorId: 'actor-1',
      channelId: 'channel-1',
...
function buildContext(
  overrides: Partial<Parameters<UnauthorizedChannelCreateDetector['evaluate']>[0]> = {},
): Parameters<UnauthorizedChannelCreateDetector['evaluate']>[0] {
  ...
  return {
    ...
    actorId: 'actor-1',
    guildId: 'guild-1',
    actionType: SecurityActionType.CHANNEL_CREATE,
...
test('authorized channel create produces CLEAN finding', async () => {
  const detector = new UnauthorizedChannelCreateDetector();
...
test('trusted administrator channel create produces CLEAN finding', async () => {
  const detector = new UnauthorizedChannelCreateDetector();
...
test('unauthorized channel create is MALICIOUS and force-blocked', async () => {
  const detector = new UnauthorizedChannelCreateDetector();

  const result = await detector.evaluate(buildContext());

  expect(result.detectorId).toBe('unauthorized-channel-create-detector');
...
test('explicit policy violation is malicious even when actor is trusted', async () => {
  ...
  expect(result.matched).toBe(true);
  expect(result.findings[0]?.reason).toContain('violates channel creation policy');
});

test('non-channel-create events are ignored', async () => {
  const detector = new UnauthorizedChannelCreateDetector();
  ...
  expect(result.matched).toBe(false);
  expect(result.findings[0]?.reason).toBe('Event does not represent channel creation');
});
...
test('detector source has no forbidden integration surfaces', () => {
  const detectorFilePath = path.join(process.cwd(), 'src/core/runtime/discord/unauthorized-channel-create-detector.ts');
