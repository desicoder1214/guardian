import {
  ...
  DiscordBotRemovalOperationRequest,
...
export interface ProductionDiscordBotRemovalOperationOptions {
...
function parseRateLimit(headers: HeaderReader): DiscordBotRemovalOperationResponse['rateLimit'] {
...
export class ProductionDiscordBotRemovalOperation implements DiscordBotRemovalOperation {
  private readonly fetchFn: FetchLike;
  private readonly apiBaseUrl: string;
  private readonly apiVersion: number;
  private readonly userAgent: string;

  constructor(private readonly options: ProductionDiscordBotRemovalOperationOptions) {
    if (!options.botToken || options.botToken.trim().length === 0) {
      throw new Error('ProductionDiscordBotRemovalOperation requires a non-empty botToken');
    }
    ...
    this.fetchFn =
      ...
      (() => {
        throw new Error('No fetch implementation available for ProductionDiscordBotRemovalOperation');
      });
  ...
  async removeUnauthorizedBot(
    request: DiscordBotRemovalOperationRequest,
  ): Promise<DiscordBotRemovalOperationResponse> {
    const endpoint = `${this.apiBaseUrl}/api/v${this.apiVersion}/guilds/${encodeURIComponent(
      request.guildId,
    )}/members/${encodeURIComponent(request.botUserId)}`;
