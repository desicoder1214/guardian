...
export class UnauthorizedChannelDeleteDetector implements DetectorPlugin {
  readonly detectorId = 'unauthorized-channel-delete-detector';
  readonly version = '1.0.0';
  readonly priority = 95;
  readonly supportedActionTypes = Object.freeze([SecurityActionType.CHANNEL_DELETE]);
  ...
  async evaluate(context: DetectionContext): Promise<DetectionResult> {
    ...
    const isChannelDeleteEvent = eventName === 'CHANNEL_DELETE';
    ...
    const channelPolicyViolation = this.readBoolean(
      ...
      'dangerousChannelDeletion',
      'dangerous_channel_deletion',
      'policyViolation',
      'policy_violation',
    ) ?? false;

    const unauthorizedChannelDeletion =
      isChannelDeleteEvent && !isAuthorizedChannel && !isAuthorizedActor && !isTrustedActor;
    const malicious = unauthorizedChannelDeletion || channelPolicyViolation;
      ...
      metadata: Object.freeze({
        ...
        runtimeThreatOverrides: malicious
          ? Object.freeze([
              Object.freeze({
                type: 'FORCE_BLOCK',
                applicableEventTypes: Object.freeze(['CHANNEL_DELETE']),
                reason: 'mandatory unauthorized channel deletion containment',
              }),
            ])
          : Object.freeze([]),
      }),
  ...
  private resolveReason(
    ...
    if (!isChannelDeleteEvent) {
      return 'Event does not represent channel deletion';
    }

    if (channelPolicyViolation) {
      return `Channel ${channelId ?? 'unknown-channel'} violates channel deletion policy`;
    }

    if (isAuthorizedChannel || isAuthorizedActor || isTrustedActor) {
      return `Channel ${channelId ?? 'unknown-channel'} deletion authorized for actor ${actorId}`;
    }

    return `Channel ${channelId ?? 'unknown-channel'} deletion is not authorized for actor ${actorId}`;
  }
