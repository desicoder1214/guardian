import {
  DiscordIntegrationRestorationOperation,
  DiscordIntegrationRestorationOperationRequest,
  DiscordIntegrationRestorationOperationResponse,
...
export interface ProductionDiscordIntegrationRestorationOperationOptions {
...
function parseRateLimit(headers: HeaderReader): DiscordIntegrationRestorationOperationResponse['rateLimit'] {
...
export class ProductionDiscordIntegrationRestorationOperation implements DiscordIntegrationRestorationOperation {
  private readonly fetchFn: FetchLike;
  private readonly apiBaseUrl: string;
  private readonly apiVersion: number;
  private readonly userAgent: string;

  constructor(private readonly options: ProductionDiscordIntegrationRestorationOperationOptions) {
    if (!options.botToken || options.botToken.trim().length === 0) {
      throw new Error('ProductionDiscordIntegrationRestorationOperation requires a non-empty botToken');
    }
    ...
    this.fetchFn =
      ...
      (() => {
        throw new Error('No fetch implementation available for ProductionDiscordIntegrationRestorationOperation');
      });
  ...
  async restoreIntegration(
    ...
    const endpoint = `${this.apiBaseUrl}/api/v${this.apiVersion}/guilds/${encodeURIComponent(
      request.guildId,
    )}/integrations/${encodeURIComponent(request.integrationId)}`;
