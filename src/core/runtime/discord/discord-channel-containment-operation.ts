import {
  DiscordChannelContainmentOperation,
  DiscordChannelContainmentOperationRequest,
...
export interface ProductionDiscordChannelContainmentOperationOptions {
...
function parseRateLimit(headers: HeaderReader): DiscordChannelContainmentOperationResponse['rateLimit'] {
...
export class ProductionDiscordChannelContainmentOperation implements DiscordChannelContainmentOperation {
  private readonly fetchFn: FetchLike;
  private readonly apiBaseUrl: string;
  private readonly apiVersion: number;
  private readonly userAgent: string;

  constructor(private readonly options: ProductionDiscordChannelContainmentOperationOptions) {
    if (!options.botToken || options.botToken.trim().length === 0) {
      throw new Error('ProductionDiscordChannelContainmentOperation requires a non-empty botToken');
    }
    ...
    this.fetchFn =
      ...
      (() => {
        throw new Error('No fetch implementation available for ProductionDiscordChannelContainmentOperation');
      });
  ...
  async containChannel(request: DiscordChannelContainmentOperationRequest): Promise<DiscordChannelContainmentOperationResponse> {
    const endpoint = `${this.apiBaseUrl}/api/v${this.apiVersion}/channels/${encodeURIComponent(request.channelId)}`;
