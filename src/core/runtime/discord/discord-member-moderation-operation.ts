import {
  ...
  DiscordMemberModerationOperationRequest,
...
export interface ProductionDiscordMemberModerationOperationOptions {
...
function parseRateLimit(headers: HeaderReader): DiscordMemberModerationOperationResponse['rateLimit'] {
...
export class ProductionDiscordMemberModerationOperation implements DiscordMemberModerationOperation {
  private readonly fetchFn: FetchLike;
  private readonly apiBaseUrl: string;
  private readonly apiVersion: number;
  private readonly userAgent: string;

  constructor(private readonly options: ProductionDiscordMemberModerationOperationOptions) {
    if (!options.botToken || options.botToken.trim().length === 0) {
      throw new Error('ProductionDiscordMemberModerationOperation requires a non-empty botToken');
    }
    ...
    this.fetchFn =
      ...
      (() => {
        throw new Error('No fetch implementation available for ProductionDiscordMemberModerationOperation');
      });
  ...
  async banMember(request: DiscordMemberModerationOperationRequest): Promise<DiscordMemberModerationOperationResponse> {
    const endpoint = `${this.apiBaseUrl}/api/v${this.apiVersion}/guilds/${encodeURIComponent(
      request.guildId,
    )}/bans/${encodeURIComponent(request.memberUserId)}`;
    return this.sendModerationRequest(endpoint, request, 'PUT');
  }

  async kickMember(request: DiscordMemberModerationOperationRequest): Promise<DiscordMemberModerationOperationResponse> {
    const endpoint = `${this.apiBaseUrl}/api/v${this.apiVersion}/guilds/${encodeURIComponent(
      request.guildId,
    )}/members/${encodeURIComponent(request.memberUserId)}`;
    return this.sendModerationRequest(endpoint, request, 'DELETE');
  }

  private async sendModerationRequest(
    endpoint: string,
    request: DiscordMemberModerationOperationRequest,
    method: 'PUT' | 'DELETE',
  ): Promise<DiscordMemberModerationOperationResponse> {
