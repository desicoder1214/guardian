import {
  DiscordPermissionOverwriteOperation,
  DiscordPermissionOverwriteOperationRequest,
...
export interface ProductionDiscordPermissionOverwriteOperationOptions {
...
function parseRateLimit(headers: HeaderReader): DiscordPermissionOverwriteOperationResponse['rateLimit'] {
...
export class ProductionDiscordPermissionOverwriteOperation implements DiscordPermissionOverwriteOperation {
  private readonly fetchFn: FetchLike;
  private readonly apiBaseUrl: string;
  private readonly apiVersion: number;
  private readonly userAgent: string;

  constructor(private readonly options: ProductionDiscordPermissionOverwriteOperationOptions) {
    if (!options.botToken || options.botToken.trim().length === 0) {
      throw new Error('ProductionDiscordPermissionOverwriteOperation requires a non-empty botToken');
    }
    ...
    this.fetchFn =
      ...
      (() => {
        throw new Error('No fetch implementation available for ProductionDiscordPermissionOverwriteOperation');
      });
  ...
  async restorePermissionOverwrite(
    ...
    const endpoint = `${this.apiBaseUrl}/api/v${this.apiVersion}/channels/${encodeURIComponent(
      request.channelId,
    )}/permissions/${encodeURIComponent(request.overwriteId)}`;
